...
- **Freemium Subscription** — plans: free / monthly ₹199 / six_month ₹999 / yearly ₹1599; 3-day trial; free caps: 5 AI/day, 1 child, 2 routines, 3 hub articles
  ...
  - **Razorpay (web + Android)** — second payment provider for India (UPI / cards / netbanking / wallets) using Razorpay Subscriptions (mandates / UPI Autopay). iOS continues to use RevenueCat / Apple IAP only.
    - Backend: `POST /api/subscription/razorpay/create-subscription` (auth required, returns `{subscriptionId, keyId}` from Razorpay), `POST /api/subscription/razorpay/verify` (HMAC-SHA256 verify of `{paymentId|subscriptionId}`, optimistic activation), `POST /api/subscription/razorpay/webhook` (verifies `X-Razorpay-Signature`, idempotent on `event.id`, mounted before global `requireAuth`). Webhook returns 503 in production when `RAZORPAY_WEBHOOK_SECRET` unset.
    - Lifecycle mapping: `subscription.activated` / `.charged` / `.resumed` / `.authenticated` → `activateSubscription(plan, periodEnd, provider="razorpay")`; `subscription.cancelled` / `.completed` / `.expired` / `.paused` → `canceled` (premium kept until `currentPeriodEnd`); `subscription.halted` → `past_due`.
