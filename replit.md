...
- **Parent Hub Backend (Progressive Content):** Stories carry an optional `ageBand` tag (untagged = universal Section-1 fallback) and Phonics carry an optional `conceptId` so Section 2 can group items by concept (one per concept_id) for age-2+ children, while infants get a flat "discovery" preview. Per-(child, band) unlocks are tracked in `child_band_unlocks` with `source ∈ {auto_dob, early_completion}`; only `early_completion` rows are persisted (DOB-derived bands are computed at read time). The early-unlock evaluator (`parentHubService.evaluateEarlyUnlock`) fires when band progress ≥ 75% (stories completed + phonics mastered) and is irreversible — un-marking items never revokes the row. The unified resolver (`resolveUnlockedBands`) OR-merges DOB-derived bands and unlock rows, so the `previewOnly` flag returned by `GET /api/hub/content` honours both pathways identically. Hooks are wired into `/api/stories/progress` and the phonics "mastered" branch via `evaluateEarlyUnlockSafe` (best-effort, never blocks the user request).
- **Parent Hub Frontend (Progressive Content rendering):** Both web (`artifacts/kidschedule/src/components/hub-progressive-content.tsx`, wired into `parenting-hub.tsx`) and mobile (`artifacts/amynest-mobile/components/HubProgressiveContent.tsx`, wired into `app/(tabs)/hub.tsx`) consume `GET /api/hub/content?childId=` and render Section 1 (live current-band + universal items) plus Section 2 (future content). Items with `previewOnly: true` are dimmed with a lock badge; tapping them opens a "Coming next" affordance (toast on web, Alert on mobile) instead of playing. Live items in either section open the existing `StoryPlayer`. The web component uses orval-generated `useGetHubContent` + `getGetHubContentQueryKey` from `@workspace/api-client-react`; mobile uses raw `useQuery` keyed `["hub-content", childId]` via `useAuthFetch`. To make `previewOnly → false` flips appear without re-login, story-progress and phonics-progress hooks (`use-stories-data.ts`, `use-phonics-data.ts`, mobile `useStoriesData.ts`) invalidate the hub-content query on every successful progress write, so the next render reflects the new early-unlock row immediately. The end-to-end chain (initial preview → 75 % completion → flip to live) is covered by `parentHubService.test.ts` ("hub-content e2e" describe block).
