...
- **Parenting Hub** — 6-section collapsible layout: (1) Ask Amy AI quick prompts, (2) Research-based Parenting Articles (15 in-app articles with modal reader, category filter, age matching, save/helpful, continue-reading, all localStorage), (3) Daily Tips (Amy AI personalization), (4) Emotional Support cards → /assistant, (5) Activities & Learning (existing infant/toddler/preschool/older content), (6) **Smart Olympiad Zone** for ages 3–15 — Daily 5 MCQs + Practice + Progress tabs, 4 subjects (Math/Science/Reasoning/GK) × 3 age bands × 3 difficulties (~108-question static bank in `lib/olympiad-questions.ts`), adaptive difficulty (4+/5 bumps up, ≤1 drops down), explanations after each answer, weekly 20-question test (Mon reset, 15 pts each + 50 bonus), points + 12 badges, rule-based Amy insight + parent guidance, persisted in localStorage as `olympiad:v1:{childId}`
