...
- **Amy Coach — Infant Parent Problems (0–2 yrs)** — new static, research-based topic group inside Amy Coach on BOTH web (`artifacts/kidschedule/src/pages/ai-coach.tsx`) and mobile (`artifacts/amynest-mobile/app/(tabs)/coach.tsx`). The existing `GOAL_CATEGORIES` got an additional `infant-problems` category whose items are generated from the dataset. Picking an infant problem item bypasses the AI question→loading→result flow (intercept via `isInfantProblemId(id)` inside `handlePickGoal`) and routes to a new `infantProblem` phase that renders 4 sections: (A) Possible Reason, (B) What You Can Do (steps), (C) Amy AI Insight, (D) Reassurance, plus a "guidance only — not medical" footer and a Back button. Tri-lingual content (en/hi/hin) is selected at render via `pickLang(text, i18n.language)`. Shared workspace lib `@workspace/infant-problems` (`lib/infant-problems/src/index.ts`) holds 10 typed `InfantProblem` entries (baby-not-sleeping, excessive-crying, feeding-issues, teething-discomfort, not-eating-solids, frequent-night-waking, always-irritable, not-reaching-milestones, separation-anxiety-infant, overstimulation) and helpers `INFANT_PROBLEMS`, `isInfantProblemId`, `getInfantProblem`, `pickLang`, `normalizeLang`. Existing Amy Coach goals/questions/result flow is untouched.
- **Infant & Toddler Hub (0–24 months)** — research-based premium hub shown in the Parenting Hub only when at least one child is < 24 months old. Shipped on BOTH web (`artifacts/kidschedule/src/components/infant-hub.tsx`) and mobile (`artifacts/amynest-mobile/components/InfantHub.tsx`). 5 glass tabs (Sleep / Feeding / Development / Behavior / Daily Care) with active glow, contextual `Amy AI Suggests` insight per tab+age (deterministic, no AI call), tip card with 👍 Helpful / 🔄 Next tip / ✔️ I tried this buttons, and a "guidance only — not medical" footer. Shared workspace lib `@workspace/infant-hub` (`lib/infant-hub/src/index.ts`) holds `INFANT_TIPS` (~25 entries × 5 categories × 4 age windows 0–3 / 3–6 / 6–12 / 12–24 mo, each with en/hi/hin baked in), `getTipsForAge()`, `getAmyInsight()`, `pickLang()`, and `isInfantHubAge()`. Tri-lingual `infant_hub` namespace added to all 6 i18n JSONs. Target child = effective child if in window, else youngest qualifying child.
